...
export const registerService = ({name, phone, password}) => new Promise(async (resolve, reject) => {
    try {
        ...
        resolve({
            err: token ? 0 : 2,
            msg: token ? 'Register is successfully !' : 'Phone nummber has been already used !',
            token: token || null
        })
    } catch (error) {
        reject(error)
...
export const loginService = ({phone, password}) => new Promise(async (resolve, reject) => {
    try {
        ...
        resolve({
            err: token ? 0 : 2,
            msg: token ? 'Login is successfully !' : response ? 'Password is wrong !' : 'Phone nummber not found !',
            token: token || null
        })
    } catch (error) {
        reject(error)
